...
const CollaborationDialog: React.FC<CollaborationDialogProps> = ({ assessmentId, isOwner }) => {
  ...
  const addCollaborator = async () => {
    ...
    if (error) {
      toast.error(error.code === '23505' ? "User already added" : "Failed to add collaborator");
    } else {
      ...
      <DialogContent className="sm:max-w-md rounded-2xl">
        <DialogHeader>
          <DialogTitle className="text-2xl font-black flex items-center gap-2">
            <UserPlus className="text-primary" /> Manage Access
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-6 py-4">
          {isOwner && (
            <div className="space-y-2">
              <Label className="font-bold text-slate-500 uppercase text-[10px]">Add by Email</Label>
          ...
          <div className="space-y-3">
            <Label className="font-bold text-slate-500 uppercase text-[10px]">Current Collaborators</Label>
            <div className="space-y-2">
              {collaborators.length === 0 ? (
                <p className="text-sm text-slate-400 italic py-4 text-center border-2 border-dashed rounded-xl">No collaborators added yet.</p>
              ) : (
                collaborators.map((c) => (
